...
Thank you for your interest in contributing to **Kore**! We welcome all contributions that help improve this project—from bug fixes and documentation improvements to new features.
...
- Ensure `explicitApi()` requirements are met (Kore requires explicit visibility and return types for public declarations).
...
**Critical Rule:** If your changes affect the public API or project behavior, you **must** update the relevant documentation.
- Update Markdown files in the `docs/` directory (Writerside documentation).
- Update KDoc comments in the source code following the project's **KDoc Guidelines**:
    - Every public class, function, and property **must** have KDoc.
    - All KDoc **must** be written in English.
    - Mandatory tags: `@param` (for all parameters), `@return` (for non-Unit functions), and `@since` (version of introduction).
...
- `fix: resolve platform detection issue on macOS`
- `docs: update platform detection guide`
- `refactor: clean up Uuid implementation`
